...
def format_bounty_alert_message(new_bounties):
    """
    Formats a notification message for new bounties.
    Correctly pluralizes 'Opportunity' and includes a count and list of bounties.
    """
    count = len(new_bounties)
    # The issue title '🎯 Bounty Alert: 3 New Opportunityies found' implies a typo.
    # This function ensures the correct spelling 'Opportunities' is used.
    opportunity_word = "Opportunity" if count == 1 else "Opportunities"
    message_header = f"🎯 Bounty Alert: {count} New {opportunity_word} found\n\n"

    message_body = []
    for bounty in new_bounties:
        # Markdown link format: [title](url)
        message_body.append(f"- [{bounty['title']}]({bounty['html_url']})")

    return message_header + "\n".join(message_body) if new_bounties else ""
...
def create_github_issue(repo_fullname, token, title, body):
    ...
    try:
        ...
        print(f"Failed to create GitHub Issue: {e}") # <-- CHANGE: Corrected truncated f-string
