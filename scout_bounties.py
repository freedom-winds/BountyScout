...
def create_github_issue(repo_fullname, token, title, body):
    ...
    # Fix common typo "Opportunityies" to "Opportunities" in the issue title
    fixed_title = title.replace("Opportunityies", "Opportunities")
    
    url = f"https://api.github.com/repos/{repo_fullname}/issues"
    payload = {
        "title": fixed_title, # Use the fixed title
        "body": body,
        "labels": ["bounty-alert"]
    }
    headers = {
