...
def create_github_issue(repo_fullname, token, title, body):
    ...
    # Fix the common typo "Opportunityies" to "Opportunities" in the title.
    # This ensures the GitHub alert itself has correct grammar,
    # regardless of how the title string was initially constructed by the caller.
    corrected_title = re.sub(r'\bOpportunityies\b', 'Opportunities', title)

    payload = {
        "title": corrected_title, # Use the corrected title
        "body": body,
        "labels": ["bounty-alert"]
    }
    headers = {
