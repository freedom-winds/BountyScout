...
import urllib.parse
import re
from datetime import datetime, timezone # Added timezone import for UTC timestamp
...
def create_github_issue(repo_fullname, token, title, body):
    ...
    # Add a UTC timestamp to the body for better context.
    current_time_utc = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    enhanced_body = f"Found on: {current_time_utc}\n\n{body}"

    # Attempt to correct a known typo in the title if it matches a specific pattern.
    # This addresses the observed 'Opportunityies' typo in issue #365.
    corrected_title = title
    if "Opportunityies" in title:
        corrected_title = title.replace("Opportunityies", "Opportunities")

    url = f"https://api.github.com/repos/{repo_fullname}/issues"
    payload = {
        "title": corrected_title, # Use corrected title
        "body": enhanced_body,    # Use enhanced body
        "labels": ["bounty-alert"]
    }
    headers = {
        "Accept": "application/vnd.github+json",
